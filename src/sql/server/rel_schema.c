...
rel_schemas(mvc *sql, symbol *s)
{
	...
	default:
		return sql_error(sql, 01, SQLSTATE(M0M03) "Schema statement unknown symbol(%p)->token = %s", (void*) s, token2string(s->token));
	}
